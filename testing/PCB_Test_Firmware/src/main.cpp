...
// Raw ESP-IDF UART driver (not HardwareSerial) — see tmc_uart_stream.h for why.
...
void setup() {
	...
	// TEMP: back to single-pin GPIO4 for both TX/RX (still via the raw IDF
	// driver) to isolate the new stronger pull-up as the only changed
	// variable. GPIO46 jumper stays physically connected, just unused here.
	tmcSerial.begin(UART_NUM_1, PIN_TMC_UART_TX, PIN_TMC_UART_TX, 115200);
	...
	bleLog("EMBO PCB test firmware ready, advertising as 'EMBO-PCB-Test'");
}
...
void loop() {
	...
	if (now - lastDebugPrint >= 1000) {
		lastDebugPrint = now;
		bleLog("[DEBUG] UAS=%.4fV SG1=%u SG2=%u pos1=%ld pos2=%ld",
	...
	// Follow it with a raw bus capture — see bus_probe.h — since a 0/non-zero
	// result alone can't distinguish "silent" from "garbled" replies.
