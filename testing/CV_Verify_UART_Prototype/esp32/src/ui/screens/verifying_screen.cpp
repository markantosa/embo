...
void VerifyingScreen::_draw(bool waiting, bool forceFull) {
    ...
    if (waiting) {
        ...
        // Two-column result layout: captured image on the left, histogram
        // + median stacked on the right — sized around whatever the image
        // actually is (RPI_IMG_MAX_W/H) rather than hardcoded, so this
        // still lays out sanely if that constant ever changes.
...
void VerifyingScreen::update(ScreenManager &mgr, bool forceFull) {
    ...
    if (status == RpiCaptureStatus::RESULT_READY) {
        ...
        // rpi_uart.h) — a real SIZE line is NOT guaranteed from the Pi
        // side yet (detection.py/sizing.py may still be unimplemented
        // there), so gotSize/median/iqr can be stale/unset here. Only
        // trust and display/feed a result when a real SIZE line actually
        // arrived, so placeholder/absent data never contaminates the
