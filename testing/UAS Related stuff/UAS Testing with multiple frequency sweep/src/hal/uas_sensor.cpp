...
static float _currentFreqHz = UAS_DDS_FREQ_HZ;
...
void uasSetFrequency(float hz) {
	...
	_currentFreqHz = hz;
	// Same settle rationale as init — let the transducer/envelope-detector
	// RC catch up to the new drive frequency before anything reads the ADC.
	delay(UAS_DDS_SETTLE_MS);
	bleLog("UAS: frequency set to %.0f Hz", hz);
}

float uasGetFrequency() {
	return _currentFreqHz;
}
