...
static void IRAM_ATTR _isrEncoder(void *arg) {
	uint8_t a = (uint8_t)gpio_get_level((gpio_num_t)PIN_EC11_A);
	uint8_t b = (uint8_t)gpio_get_level((gpio_num_t)PIN_EC11_B);
...
void ui_init() {
	...
	gpio_set_intr_type((gpio_num_t)PIN_EC11_A, GPIO_INTR_ANYEDGE);
	gpio_set_intr_type((gpio_num_t)PIN_EC11_B, GPIO_INTR_ANYEDGE);

	// Shared across the app; harmless (ESP_ERR_INVALID_STATE) if something
	// else installs it first — no other GPIO ISR user in this firmware.
	esp_err_t err = gpio_install_isr_service(0);
	if (err != ESP_OK && err != ESP_ERR_INVALID_STATE) {
		// Nothing else to do here — logged via bleLog would create a layering
		// dependency ui.cpp doesn't otherwise need; silently continue, the
		// encoder just won't work if this genuinely failed.
	}
	gpio_isr_handler_add((gpio_num_t)PIN_EC11_A, _isrEncoder, nullptr);
	gpio_isr_handler_add((gpio_num_t)PIN_EC11_B, _isrEncoder, nullptr);
