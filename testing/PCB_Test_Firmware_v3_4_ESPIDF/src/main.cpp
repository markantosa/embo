...
static void appSetup() {
	...
	// TMC diagnostics below over the LOG characteristic, not just on the IDF log.
	bleServiceInit();
	...
	// Status LED starts blinking here (see updateStatusLed() in appLoop()) and
...
static void appLoop() {
	...
	// Raw sensor dump, streamed over BLE (and the IDF log) — for bring-up only.
