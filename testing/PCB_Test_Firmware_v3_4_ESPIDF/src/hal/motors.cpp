...
void StepperAxis::begin(uint8_t uartAddr, int stepPin, int dirPin,
	...
	tmc2209::configureDriver(_uartAddr, runCurrentMa, holdCurrentMa);
...
bool StepperAxis::confirmSpreadCycle() {
	return tmc2209::confirmSpreadCycle(_uartAddr);
}

uint8_t StepperAxis::testConnection() {
	return tmc2209::testConnection(_uartAddr);
}
...
uint16_t StepperAxis::readStallGuard() {
	return tmc2209::readStallGuard(_uartAddr);
}
...
void motorsInit() {
	tmc2209::busInit(UART_NUM_1, PIN_TMC_UART_TX, PIN_TMC_UART_RX, 115200);
	motorM1.begin(TMC_ADDR_M1, PIN_STEP_M1, PIN_DIR_M1, PIN_EN_M1, 0,
	              TMC_RUN_CURRENT_MA, TMC_HOLD_CURRENT_MA);
	motorM2.begin(TMC_ADDR_M2, PIN_STEP_M2, PIN_DIR_M2, PIN_EN_M2, 1,
	              TMC_RUN_CURRENT_MA, TMC_HOLD_CURRENT_MA);
}
