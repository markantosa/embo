...
// Native ESP-IDF NimBLE host implementation (replaces h2zero NimBLE-Arduino).
...
// Formats like printf, sends to the IDF log AND (if a client is subscribed)
// as a BLE notification on the LOG characteristic — lets the web dashboard
// show firmware debug output without a serial monitor.
void bleLog(const char *fmt, ...);

// Set by the BLE write callbacks, drained from the main loop so motor/homing
// logic never runs inside the NimBLE host task context.
