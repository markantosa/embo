...
firmware/esp32/include/rpi_uart.h):
  ...
  Pi -> ESP32:  "SIZE <median_um> <iqr_um>\n"         (reply)
             or any other line, treated as a free-form status/diagnostic —
                ...
                isn't a valid SIZE line, so status strings are always safe
                to send.

Firmware gives up and marks the request TIMED_OUT if no SIZE reply arrives
within RPI_CAPTURE_TIMEOUT_MS (firmware/esp32/include/config.h, currently
8000ms) — see CAPTURE_TIMEOUT_S in config.py. Capture + inference must fit
inside that budget; this module does not enforce it, main.py does (the link
itself just sends/receives lines).
"""
import serial
