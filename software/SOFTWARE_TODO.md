...
- [ ] Requires strobed, short-duration illumination synced to the camera trigger — flow velocity through the narrow bore is much higher than in the barrel, so continuous lighting will motion-blur
- [ ] **UPDATE — higher risk now than originally scoped.** The original plan assumed a global-shutter sensor, where strobing mainly compensates for flow velocity. The USB microscope camera now in use is very likely **rolling shutter**, which adds frame skew/distortion on top of blur for anything moving during readout — strobe timing and exposure duration need to be tight enough to freeze motion within a single row-readout window, not just "short enough to avoid blur." Test this specifically before committing to fabrication; it may turn out this camera isn't fast/controllable enough for clean stopcock imaging at all.
